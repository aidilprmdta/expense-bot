...
def _safe_int(v) -> int:
    """
    Parse nilai Harga jadi int dengan aman.

    PENTING: get_all_records() dipanggil dengan numericise_ignore=["all"],
    jadi SEMUA kolom (termasuk Harga) balik sebagai STRING mentah dari
    Sheets API, bukan angka asli. Kalau cell diformat dengan pemisah
    ribuan (misal locale Indonesia: "25.000"), `int(r.get("Harga", 0))`
    langsung akan CRASH dengan ValueError. Fungsi ini menangani semua
    kemungkinan format: "25000", "25.000", "25,000", "Rp 25.000", dst.
    Konsisten dengan _safe_int() yang sudah dipakai di rekap.py, hapus.py,
    edit.py, dan handler lain.
    """
...
# ─────────────────────────────────────────────────────────────
# TRANSAKSI BERULANG (RUTIN) — disimpan di tab "Rutin" terpisah,
# tiap baris = satu aturan tagihan rutin (listrik, internet, dll)
# yang otomatis dicatat tiap tanggal tertentu tiap bulan.
# ─────────────────────────────────────────────────────────────

RUTIN_SHEET_NAME = "Rutin"
RUTIN_HEADERS    = ["ChatID", "Nama", "Harga", "Kategori", "Hari", "Catatan"]

_cached_rutin_sheet: Optional[gspread.Worksheet] = None


def _connect_to_rutin_sheet() -> gspread.Worksheet:
    """Buka (atau buat) worksheet 'Rutin' di spreadsheet yang sama."""
    _get_sheet()  # reuse auth
    spreadsheet_id = os.getenv("SPREADSHEET_ID")
    creds          = _load_credentials()
    client         = gspread.authorize(creds)
    workbook       = client.open_by_key(spreadsheet_id)
    ...
    try:
        rutin_sheet = workbook.worksheet(RUTIN_SHEET_NAME)
    except gspread.exceptions.WorksheetNotFound:
        rutin_sheet = workbook.add_worksheet(
            title=RUTIN_SHEET_NAME, rows=100, cols=len(RUTIN_HEADERS)
        )
        rutin_sheet.update("A1", [RUTIN_HEADERS])
        rutin_sheet.format(
            f"A1:{chr(64 + len(RUTIN_HEADERS))}1",
            {
                "textFormat": {"bold": True, "foregroundColor": {"red": 1, "green": 1, "blue": 1}},
                "backgroundColor": HEADER_BG_COLOR,
            },
        )
        logger.info(f"[sheets] Tab '{RUTIN_SHEET_NAME}' dibuat.")

    return rutin_sheet


def _get_rutin_sheet() -> gspread.Worksheet:
    global _cached_rutin_sheet
    if _cached_rutin_sheet is not None:
        return _cached_rutin_sheet
    _cached_rutin_sheet = _connect_to_rutin_sheet()
    return _cached_rutin_sheet


def _sync_get_all_rutin() -> list[dict]:
    sheet = _get_rutin_sheet()
    return sheet.get_all_records(numericise_ignore=["all"])


def _sync_add_rutin(row: list) -> None:
    sheet = _get_rutin_sheet()
    sheet.append_row(row)


def _sync_delete_rutin(row_number: int) -> None:
    sheet = _get_rutin_sheet()
    sheet.delete_rows(row_number)


async def get_all_rutin() -> list[dict]:
    """Ambil semua aturan transaksi rutin (baris 2 dst di tab Rutin)."""
    return await asyncio.to_thread(_sync_get_all_rutin)


async def add_rutin(chat_id: int, nama: str, harga: int, kategori: str, hari: int, catatan: str = "-") -> None:
    """Tambah satu aturan transaksi rutin baru."""
    row = [chat_id, nama, harga, kategori, hari, catatan]
    await asyncio.to_thread(_sync_add_rutin, row)


async def delete_rutin(row_number: int) -> None:
    """Hapus satu aturan transaksi rutin berdasarkan nomor baris di tab Rutin."""
    if row_number < 2:
        raise ValueError("Nomor baris tidak valid — baris 1 adalah header.")
    await asyncio.to_thread(_sync_delete_rutin, row_number)
...
# ─────────────────────────────────────────────────────────────
# LANGGANAN LAPORAN MINGGUAN — daftar chat_id yang mau dikirimi
# ringkasan pengeluaran otomatis tiap Senin pagi. Disimpan di tab
# "Config" yang sama, format comma-separated chat_id.
# ─────────────────────────────────────────────────────────────

LAPORAN_MINGGUAN_KEY = "LAPORAN_MINGGUAN_CHATS"


def _sync_get_weekly_subscribers() -> list[int]:
    """Baca daftar chat_id yang berlangganan laporan mingguan."""
    try:
        sheet = _get_config_sheet()
        cell  = sheet.find(LAPORAN_MINGGUAN_KEY)
        if cell is None:
            return []
        value = sheet.cell(cell.row, cell.col + 1).value or ""
        hasil = []
        ...
        logger.warning(f"[sheets] Gagal baca subscriber laporan mingguan: {e}")
        return []


def _sync_save_weekly_subscribers(chat_ids: list[int]) -> None:
    """Simpan ulang seluruh daftar subscriber (overwrite)."""
    sheet = _get_config_sheet()
    value = ",".join(str(c) for c in chat_ids)
    cell  = sheet.find(LAPORAN_MINGGUAN_KEY)
    if cell is None:
        sheet.append_row([LAPORAN_MINGGUAN_KEY, value])
    else:
        sheet.update_cell(cell.row, cell.col + 1, value)


async def get_weekly_subscribers() -> list[int]:
    """Ambil daftar chat_id yang berlangganan laporan mingguan otomatis."""
    return await asyncio.to_thread(_sync_get_weekly_subscribers)


async def add_weekly_subscriber(chat_id: int) -> bool:
    """
    Daftarkan chat_id untuk terima laporan mingguan.
    Return True kalau baru ditambahkan, False kalau sudah langganan.
    """
    current = await get_weekly_subscribers()
    if chat_id in current:
        return False
    current.append(chat_id)
    await asyncio.to_thread(_sync_save_weekly_subscribers, current)
    return True


async def remove_weekly_subscriber(chat_id: int) -> bool:
    """
    Berhenti langganan laporan mingguan.
    Return True kalau berhasil dihapus, False kalau memang belum langganan.
    """
    current = await get_weekly_subscribers()
